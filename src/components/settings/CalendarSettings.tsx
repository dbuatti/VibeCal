...
const ProviderIcon = ({ provider }: { provider: string }) => {
  ...
  if (provider === 'apple') {
    return (
      <svg viewBox="0 0 24 24" className="w-4 h-4 text-gray-900 shrink-0" fill="currentColor">
            ...
            <div className="px-6 pb-6 space-y-8 min-w-0">
              {providers.length > 0 ? providers.map((provider) => (
                <div key={provider} className="space-y-4 min-w-0">
                  <div className="flex items-center justify-between sticky top-0 bg-white py-2 z-10 border-b border-gray-50 gap-2">
